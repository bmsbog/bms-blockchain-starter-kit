...
describe('Starting tests for bms-angular-app', function() {
  ...
  it('website title should be bms-angular-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('bms-angular-app');
    })
  });
  ...
  it('navbar-brand should be bms-angular-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('bms-angular-app');
    });
  });
